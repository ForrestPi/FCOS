...
loader_train = torch.utils.data.DataLoader(dataset_train, batch_size=cfg['nbatch_train'], 
                    shuffle=True, num_workers=cfg['num_workers'], pin_memory=True,collate_fn=dataset_train.collate_fn)
...
# Prepare engine structure
trainer = engine.Trainer(net, dataset_train, loader_train, cfg['device'], opt, cfg['grad_clip'], lr_func)
evaluator = engine.Evaluator(net, dataset_eval, loader_eval, cfg['device'])
if cfg['load']:
